...
const ParameterCard = ({ paramName, value, lastUpdated }) => {
  const config = parameterConfig[paramName];
  if (!config) return null;
  const status = getStatus(paramName, value);
  const colors = statusColors[status];
  const sectionColor = sectionColors[config.section];

  // Calculate fill percentage for mini progress bar
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      try {
        ...
        setActiveAlerts(res.data.data.length);
      } catch (err) {
  ...
  const criticalCount = Object.entries(readings).filter(
    ([name, val]) => getStatus(name, val) === 'critical'
  ).length;

  const warningCount = Object.entries(readings).filter(
    ([name, val]) => getStatus(name, val) === 'warning'
  ).length;
